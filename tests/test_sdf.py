...
class Test(unittest.TestCase):
    ...
    def test_3D_example(self):
        ...
        w = (
            np.concatenate(
                (np.arange(0.0, 500.0, 100), np.arange(500.0, 12e3 + 500, 500))
            )
            * RPM2RADS
        )
        ...
        P_loss = (
            kfric * W + kfric3 * W**3 + (res * (TAU / psi) ** 2) + k_u * (U - v_ref)
        )
    ...
    def test_validate_dataset(self):
        ...
        errors = sdf._validate_dataset(ds1)
        self.assertEqual(
            ["The number of scales does not match the number of dimensions"], errors
        )
